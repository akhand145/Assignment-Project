...
const getManyByCondition = async (
  Model,
  condition,
  sortCondition,
  skip,
...
const getDeviceTokens = async (Model, condition) => {
  try {
    const data = await Model.find(condition, { _id: 0, userId: 1, deviceToken: 1 }).lean();
    return data || null;
  } catch (error) {
    return false;
  }
...
const getMany = async (Model, condition) => {
  try {
...
const updateCountManyByCondition = async (Model, condition, count) => {
  try {
